...
face_locations_known = face_recognition.face_locations(
    face_recognition.load_image_file(IMAGE_LOCATION_KNOWN)
)
face_locations_unknown = face_recognition.face_locations(
    face_recognition.load_image_file(IMAGE_LOCATION_UNKNOWN)
)
...
class Window(Frame):
    def __init__(self, master=None):
        ...
        cropped_image_known = image_known.crop(
            [left1 - 20, top1 - 20, right1 + 20, bottom1 + 20]
        )
        cropped_image_unknown = image_unknown.crop(
            [left2 - 20, top2 - 20, right2 + 20, bottom2 + 20]
        )

        cropped_image_known = cropped_image_known.resize(
            (IMAGE_SCALE, IMAGE_SCALE), Image.ANTIALIAS
        )
        cropped_image_unknown = cropped_image_unknown.resize(
            (IMAGE_SCALE, IMAGE_SCALE), Image.ANTIALIAS
        )
        ...
        text = Label(
            width=30,
            height=2,
            text="SAME PERSON" if results[0] else "NOT SAME PERSON",
            font="Arial 25",
            padx=0,
            pady=0,
        )
